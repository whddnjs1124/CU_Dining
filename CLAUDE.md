...
Columbia Dining 대시보드. dining.columbia.edu를 3시간마다 스크레이핑해서 정적 JSON으로 굽고, 한 페이지 대시보드로 보여준다. 아이폰 홈 화면에 설치 가능(PWA).

기획은 `docs/PRD.md`, 설계와 실패 모드는 `docs/HLD.md`.

**이 파일은 200줄을 넘기지 않는다.** 뭔가 바꿨으면 여기 서술도 같이 고칠 것 — 특히 주기·임계값·테스트 개수처럼 숫자가 박힌 문장. 길어지면 상세 설명은 `docs/HLD.md`로 내리고 여기엔 규칙만 남긴다. 낡은 서술은 없는 것보다 나쁘다.
...
**열림/닫힘은 프론트에서 계산한다.** `dining.json`에는 시간대만 담고 상태 문자열은 담지 않는다. JSON이 몇 시간 stale해도 상태 표시는 정확해야 하니까.
...
**크롤은 예의 있게, 그리고 드물게.** cron은 3시간 주기(하루 8회)다. 30분 주기였을 때 **185회 연속 스크레이프에서 `updated_at` 말고는 아무것도 안 바뀌었고**, 그러다 Cloudflare가 러너를 통째로 막았다. 컬럼비아는 메뉴를 몇 달치 미리 발행하니(9/4~11/1치가 8월에 이미 존재) 잦게 돌 이유가 없다. 더 촘촘하게 바꾸지 말 것.

**스크레이퍼는 자동화 티를 내지 않는다.** `navigator.webdriver=true`와 UA의 `HeadlessChrome`이 실측된 신호였고 `open_page()`가 둘 다 처리한다. UA는 하드코딩하지 말 것 — 실제 UA에서 "Headless"만 떼야 `navigator.platform`과 모순이 안 생긴다.

**주기와 stale 배너 임계값은 한 쌍이다.** 지금 3시간 주기 / 12시간 경고. 주기를 바꾸면 `index.html`의 `STALE_AFTER_HOURS`도 같이 봐야 한다 — 한 번 밀린 게 경고로 뜨면 배너가 소음이 된다.
...
배포됨: <https://whddnjs1124.github.io/CU_Dining/>. 되돌아갈 지점은 태그 `v1.0-web`.

**미검증 하나 — 메뉴 렌더링.** 여름방학이라 `menu_data`가 비어 있다(다이닝홀 전부 `Closed for Summer`, Blue Java 3곳만 평일 운영). 첫 Fall 메뉴 노드는 **2026-09-04**자이고, 그날 실물로 확인해야 한다. 프론트·스크레이퍼 양쪽 다 합성 페이로드로만 검증됐다. `scrape.py`의 `build_item()`에 `ponytail:` 코멘트로 표시해 뒀다 — 끼니 항목의 필드명은 앱 JS의 필터 구현에서 역산한 것이라 실제와 다를 수 있다.

**지켜보는 중 — Cloudflare.** 8/18에 러너가 통째로 차단됐다가 지문 수정(`open_page()`)으로 복구됐다. 아직 성공 사례가 많지 않으니, `scrape` 실패 메일이 오면 주기를 더 낮추는 게 다음 카드다. 실패 이유는 이제 알림 본문에 찍힌다(`die()`).
