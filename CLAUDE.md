...
**의존성을 늘리지 않는다.** 프론트는 프레임워크·번들러·CDN 스크립트 없이 `index.html` 한 파일이다. Python 쪽은 `playwright`, `beautifulsoup4`, `pytest`가 전부. 새 패키지를 추가하려면 그게 없으면 못 하는 일인지 먼저 따져볼 것.

**부분 결과를 절대 커밋하지 않는다.** 스크레이프가 실패하거나 로케이션 수가 이전보다 줄면 non-zero exit로 죽어라. 조금 오래된 정확한 데이터 > 방금 만든 반쪽 데이터. 프론트는 stale을 배너로 알리지만, 틀린 데이터는 알릴 방법이 없다.
...
**크롤은 예의 있게.** 페이지 간 2초 이상 간격, 30분보다 잦은 cron 금지. Cloudflare 뒤에 있는 남의 사이트다.
...
- `index.html?selftest` — 열림/닫힘 로직 어서션 13개를 브라우저에서 실행.
