...
`index.html` 한 파일. 프레임워크·번들러·CDN 스크립트 없음.

- `fetch('dining.json')` → 렌더.
- 뉴욕 현재 시각은 `Intl.DateTimeFormat(..., { timeZone: 'America/New_York' })`. DST를 브라우저가 처리하므로 날짜 라이브러리 불필요.
- 상태 4종 `open` / `closing`(1시간 내 마감) / `opening`(1시간 내 개점) / `closed`, 이 순서로 정렬.
- 다이닝홀과 카페/리테일을 별도 섹션으로. 카드: 이름 / 상태 pill(색 + 텍스트 병기) / 실제 시각("Open until 9:00 PM") / 사이트 원문 시간 안내 / `<details>`로 접은 오늘 메뉴.
