...
- 카드: 이름 / 상태 pill / 실제 시각("Open until 9:00 PM") / **끼니별 접이식 메뉴**.
...
- 상단: "updated N분 ago". `updated_at`이 `STALE_AFTER_HOURS`(12시간)를 넘으면 경고 배너.
- **메뉴는 끼니마다 별도 `<details>`다.** 하루 전체를 하나로 묶으면 점심과 저녁을 나란히 못 본다 — 그런데 학생이 실제로 하는 판단이 바로 그 둘 사이의 선택이다. 여러 개를 동시에 열 수 있고, 지금 제공 중인 끼니는 기본으로 열린 채 `now` 태그가 붙는다.
...
### 주기를 3시간으로 낮춘 근거 (2026-08-18)

30분 주기는 두 가지 이유로 틀렸다.

**아무것도 안 바뀌었다.** 커밋된 `dining.json` 185건을 `updated_at`을 제외하고 해시 비교한 결과, **실질 내용이 바뀐 횟수는 0이었다.** 185번 커밋해서 0비트를 발행한 셈이다(방학이라 극단적이긴 하다).

**메뉴는 애초에 자주 안 바뀐다.** 컬럼비아는 학기 메뉴를 몇 달치 일괄 발행한다 — 8월 중순 시점에 **9월 4일~11월 1일치 208개 노드**가 이미 존재했다. 매일 갱신되는 데이터가 아니라 배치로 올라오는 데이터다.

그리고 대가가 있었다: 하루 48회 접속이 Cloudflare의 봇 탐지를 자극해 결국 러너가 통째로 차단됐다. 지금은 `0 */3 * * *`(UTC), 뉴욕 기준 대략 아침·점심·저녁에 걸친다.

**주기와 stale 임계값은 한 쌍이다.** 3시간 주기 + GitHub의 최대 1시간 지연이면 한 번 밀린 게 7시간이 될 수 있다. 그래서 경고는 12시간(`STALE_AFTER_HOURS`)으로 뒀다. 6시간이면 정상 운영 중에도 배너가 떠서 경고가 소음이 된다.
...
- **크롤 예의**: 3시간 주기, 하루 8회. `robots.txt` 준수(`/content/*` 허용됨).
